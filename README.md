# LeetCode Progress Tracker

## Leetcode Profile - [Hemnathkk](https://leetcode.com/u/hemnath_kk/)

## GitHub DSA - [DSA-Problems](https://github.com/hemnath-kandasamyk/Data-Structures-and-Algorithms)

---

## 23-05-2026

1. LC-946 - Validate Stack Sequences

   * Topic: Stack
   * Difficulty: Medium

---

## 24-05-2026

1. LC-13 - Roman to Integer

   * Topic: Strings
   * Difficulty: Easy

2. LC-02 - Add Two Numbers

   * Topic: Linked List
   * Difficulty: Medium

---

## 25-05-2026

1. LC-561 - Array Partition

   * Topic: Arrays, Greedy, Sorting
   * Difficulty: Easy

2. LC-07 - Reverse Integer

   * Topic: Math
   * Difficulty: Medium

---

## 28-05-2026

1. LC-977 - Squares of a Sorted Array

   * Topic: Arrays, Two Pointers, Sorting
   * Difficulty: Easy

---

## 29-05-2026

1. LC-3300 - Minimum Element After Replacement With Digit Sum

   * Topic: Arrays, Math
   * Difficulty: Easy

2. LC-1945 - Sum of Digits of String After Convert

   * Topic: Strings, Simulation, Math
   * Difficulty: Easy

---

## 30-05-2026

1. LC-905 - Sort Array By Parity

   * Topic: Arrays, Two Pointers
   * Difficulty: Easy

---

## 31-05-2026

1. LC-2126 - Destroying Asteroids

   * Topic: Greedy, Sorting, Array
   * Difficulty: Medium

2. LC-2368 - Reachable Nodes With Restrictions

   * Topic: Graph, BFS, DFS, Tree
   * Difficulty: Medium

---
## 02-06-2026

1. LC-1971 - Find if Path Exists in Graph

   * Topic: Graph, BFS, DFS
   * Difficulty: Easy

2. LC-841 - Keys and Rooms

   * Topic: Graph, BFS, DFS
   * Difficulty: Medium

---


